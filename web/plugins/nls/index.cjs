 ...
 * @returns
 */
function esbuildPluginMonacoEditorNls(options) {
 ...
 * @returns
 */
function rollupPluginMonacoEditorNls(options) {
	options = Object.assign({ locale: Languages.en_gb }, options);
	const CURRENT_LOCALE_DATA = getLocalizeMapping(options.locale, options.localeData);
	...
	return {
		...
		transform(code, filepath) {
			if (/monaco-editor[/\\]esm[/\\]vs.+\.js/.test(filepath) && !/esm[/\\]vs[/\\].*nls\.js/.test(filepath)) {
 ...
 * @returns
 */
function getLocalizeMapping(locale, localeData) {
...
// 5. CommonJS 导出
module.exports = {
	default: rollupPluginMonacoEditorNls,
	Languages,
