...
func (s *sStatistics) GetOverview(ctx context.Context, req *statisticsApi.OverviewReq) (*statisticsApi.OverviewRes, error) {
	...
	res.TotalVariables, err = dao.TemplateVariables.Ctx(ctx).Count()
	if err != nil {
		return nil, err
	}
	...
	// 包含变量的模板数
	variableTemplateCount, err := dao.TemplateVariables.Ctx(ctx).
		Fields("DISTINCT template_id").
		Count()
	if err != nil {
		return nil, err
	}
	res.TemplatesWithVariables = variableTemplateCount
...
func (s *sStatistics) GetTemplateComplexity(ctx context.Context, req *statisticsApi.TemplateComplexityReq) (*statisticsApi.TemplateComplexityRes, error) {
	...
	err = dao.TemplateVariables.Ctx(ctx).
		Fields("template_id, COUNT(*) as variable_count").
		Group("template_id").
		Scan(&variableCountData)
	if err != nil {
		return nil, err
	}
